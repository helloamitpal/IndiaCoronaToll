    ...
    const uniqObjList = dataset.reduce((acc, { address, latlong, _cn6ca, modeoftravel, timefrom, timeto, datasource, accuracylocation }) => {
      ...
      if (addr && latlong && !acc.hasOwnProperty(addr)) {
        acc[addr] = {
          ...
          timefrom: (timefrom && moment(timefrom, config.INPUT_DATE_FORMAT).format(config.ALTERNATE_DATE_FORMAT)) || null,
          timeto: (timeto && moment(timeto, config.INPUT_DATE_FORMAT).format(config.ALTERNATE_DATE_FORMAT)) || null,
          value: _cn6ca,
          label: addr,
          address: addr,
          zoom: 13,
